...
class WooCommerce_Gateway_Genoapay extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_title       = __( 'LatitudePay', 'wc-genoapay' );
		$this->supports           = array(
			'products',
			'refunds',
		);
		$this->order_button_text  = __( 'Proceed to LatitudePay', 'wc-genoapay' );
		...
		if ( WooCommerce_Gateway_Genoapay_API_Handler::get_auth_token() ) {
			...
			if ( ! $this->validate_currency() || $this->validate_min_max_amount() ) {
				$this->log( 'LatitudePay gateway disabled: Invalid currency or Order amount not within minimum and maximum number', 'error' );
				$this->enabled = 'no';
			} 
			
		} else {
			$this->log( 'LatitudePay gateway disabled: No auth token', 'error' );
			$this->enabled = 'no';
		}

		// Always enable the IPN handler for payment callbacks to be processed
		include_once( GENOAPAY_PLUGIN_DIR . '/includes/class-woocommerce-gateway-latitudepay-ipn-handler.php' );
	...
	public function payment_scripts() {
		...
		if ( $this->display_in_modal ) {
			...
			wp_register_script( 'genoapay-script', GENOAPAY_PLUGIN_URL . 'assets/js/latitudepay-checkout.js', array( 'jquery', 'woocommerce', 'wc-country-select', 'wc-address-i18n' ), GENOAPAY_VERSION );
	...
	public function init_form_fields() {
		...
		$this->form_fields = apply_filters( 'wc_qcard_form_fields', array(
			...
			'enabled' => array(
				...
				'type'    => 'checkbox',
				'label'   => __( 'Enable LatitudePay', 'wc-genoapay' ),
				'default' => 'yes',
			),
			'sandbox' => array(
				'title' => __( 'Sandbox', 'wc-genoapay' ),
				'label' => __( 'Enable LatitudePay sandbox', 'wc-genoapay' ),
				'type' => 'checkbox',
			...
			'debug' => array(
				...
				'default'     => 'no',
				'description' => sprintf( __( 'Log LatitudePay events, such as IPN requests, inside %s', 'wc-genoapay' ), '<code>' . WC_Log_Handler_File::get_log_file_path('LatitudePay') . '</code>' ),
			),
//			'display_in_modal' => array(
//				'title'       => __( 'Display in modal', 'wc-genoapay' ),
//				'label'       => __( 'Enable modal Checkout', 'wc-genoapay' ),
//				'type'        => 'checkbox',
//				'description' => __( 'If enabled, open LatitudePay payment in modal instead of redirect to the page.', 'wc-genoapay' ),
//				'default'     => 'no',
//			),
			'title' => array(
				...
				'default'     => __( 'LatitudePay', 'wc-genoapay' ),
				'desc_tip'    => true,
			),
			'client_key' => array(
	...
	public function get_icon() {
		...
		$icon_html .= '<img src="' . GENOAPAY_PLUGIN_URL . 'assets/images/latitudepay.png' . '" width="100" alt="' . esc_attr__( 'LatitudePay', 'wc-genoapay' ) . '" />';
	...
	protected function init_api() {
		include_once( GENOAPAY_PLUGIN_DIR . '/includes/class-woocommerce-gateway-latitudepay-api-handler.php' );
	...
	public function process_payment( $order_id ) {
		include_once( GENOAPAY_PLUGIN_DIR . '/includes/class-woocommerce-gateway-latitudepay-request.php' );
	...
	public function validate_min_max_amount() {
		...
		} elseif( is_product() && is_object($product)) {
		    try {
	...
	public static function log( $message, $level = 'info' ) {
		if ( self::$log_enabled ) {
			...
			self::$log->log( $level, $message, array( 'source' => 'LatitudePay') );
		}
	}
}
