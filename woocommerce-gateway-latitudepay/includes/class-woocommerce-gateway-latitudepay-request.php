...
class WooCommerce_Gateway_Genoapay_Request {
	...
	public function get_request_url( $order, $display_in_modal ) {
		...
		if ( WooCommerce_Gateway_Genoapay_API_Handler::get_auth_token() ) {
			...
			$return_url = add_query_arg( array(
							'wc-api' => 'WooCommerce_Gateway_LatitudePay',
							'_wpnonce' => $nonce,
			...
			WooCommerce_Gateway_Genoapay::log( 'LatitudePay Request Args for sale ' . $order->get_order_number() . ': ' . wc_print_r( $sale, true ) );
	...
	public function request_refund( $order, $amount, $reason ) {
		...
		if ( WooCommerce_Gateway_Genoapay_API_Handler::get_auth_token() ) {
			...
			WooCommerce_Gateway_Genoapay::log( 'LatitudePay Request Args for refund ' . $order->get_order_number() . ': ' . wc_print_r( $refund, true ) );
