...
class WooCommerce_Gateway_Genoapay_IPN_Handler {
	...
	public function __construct() {
		add_action( 'woocommerce_api_woocommerce_gateway_latitudepay', array( $this, 'check_response' ) );
		add_action( 'valid-latitudepay-standard-ipn-request', array( $this, 'valid_response' ) );
	}
	...
	public function check_response() {
		 ...
		 if ( isset( $_REQUEST['_wpnonce'], $_REQUEST['reference'] ) && wp_verify_nonce( sanitize_key( $_REQUEST['_wpnonce'] ), 'genoapay_' . sanitize_text_field( wp_unslash( $_REQUEST['reference'] ) ) ) ) {
			...
			do_action( 'valid-latitudepay-standard-ipn-request', $_REQUEST );
			// @codingStandardsIgnoreEnd
			exit;
		 }

		 wp_die( 'LatitudePay IPN Request Failure', 'LatitudePay IPN', array(
		 	'response' => 500,
		 ) );
	}
	...
	function valid_response( $request ) {
		try {
			...
			wp_die( 'LatitudePay IPN Request Failure::'. $e->getMessage(), 'LatitudePay IPN', array(
				'response' => 500,
			) );
		}
	}
